...
public class LetsEncryptProvider implements ISSLProvider {
  ...
  /**
   * @param sslConfig The configuration details for the provider.
   *                  {@link LetsEncryptConfig}.
   */
  public LetsEncryptProvider(LetsEncryptConfig sslConfig) {
    config = sslConfig;
  }
  ...
  public SSLContext getContext() {
    try {
      ...
      byte[] keyBytes = parseDERFromPEM(getBytes(config.privKeyPath),
          "-----BEGIN PRIVATE KEY-----", "-----END PRIVATE KEY-----");

      Certificate[] certs = getCertificateChain(config.certChainPath);
